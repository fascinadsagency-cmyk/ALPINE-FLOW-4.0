...
export default function Returns() {
  ...
  const openChangeModal = (rental, item = null) => {
    ...
    setChangeAction(item ? "swap" : "extend");
  ...
  const closeChangeModal = () => {
    ...
    setChangeOldItem(null);
    setChangeNewItem(null);
    setChangeDelta(null);
    setChangeComplete(false);
  };

  const searchChangeItem = async (code) => {
    if (!code.trim() || !changeRental) return;
    ...
    try {
      ...
      setChangeNewItem(foundItem);

      // Auto-match by type if no old item selected
      if (!changeOldItem) {
        const rentalItems = changeRental.pending_items || changeRental.items?.filter(i => !i.returned) || [];
        const matchingItem = rentalItems.find(i => 
          i.item_type?.toLowerCase() === foundItem.item_type?.toLowerCase()
        );
        if (matchingItem) {
          setChangeOldItem(matchingItem);
      ...
      await calculateChangeDelta(changeOldItem, foundItem);
  ...
  const calculateChangeDelta = async (oldItem, newItem) => {
    ...
    try {
      ...
      // Calculate old price (for remaining days)
      let oldPrice = 0;
      if (oldItem) {
        const oldTariff = tariffs.find(t => t.item_type?.toLowerCase() === oldItem.item_type?.toLowerCase());
        const oldDayField = changeDaysRemaining <= 10 ? `day_${changeDaysRemaining}` : 'day_11_plus';
        oldPrice = oldTariff?.[oldDayField] || oldItem.unit_price || 0;
      }
      ...
      if (newItem) {
        ...
        const newDayField = changeDaysRemaining <= 10 ? `day_${changeDaysRemaining}` : 'day_11_plus';
        newPrice = newTariff?.[newDayField] || newItem.rental_price || 0;
      }

      // If extending days, add the extension cost
      let extensionCost = 0;
      if (newDaysNum > oldDaysNum) {
        const extraDays = newDaysNum - oldDaysNum;
        // Calculate price per day from original rental
        const pricePerDay = changeRental.total_amount / oldDaysNum;
        extensionCost = pricePerDay * extraDays;
      }

      const materialDelta = newItem ? (newPrice - oldPrice) : 0;
      const totalDelta = materialDelta + extensionCost;
      ...
      setChangeDelta({
        ...
        isDowngrade: totalDelta < 0,
        isEqual: totalDelta === 0
      });
    } catch {
      setChangeDelta({ oldPrice: 0, newPrice: 0, delta: 0, isEqual: true });
    }
  };
  ...
  useEffect(() => {
    if (changeModal && changeRental) {
      calculateChangeDelta(changeOldItem, changeNewItem);
    }
  }, [changeNewDays]);
  ...
  const executeChange = async () => {
    ...
    // MANDATORY: Must have a delta calculated and payment flow
    if (changeDelta && changeDelta.delta !== 0) {
      // Continue with change
    } else if (!changeNewItem && parseInt(changeNewDays) === changeRental.days) {
    ...
    setChangeLoading(true);
    try {
      // If there's a material swap
      if (changeNewItem && changeOldItem) {
        await axios.post(`${API}/rentals/${changeRental.id}/central-swap`, {
          old_item_barcode: changeOldItem.barcode || changeOldItem.internal_code,
          new_item_barcode: changeNewItem.barcode || changeNewItem.internal_code,
          days_remaining: changeDaysRemaining,
          payment_method: changePaymentMethod,
          delta_amount: changeDelta?.delta || 0
        }, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
      }

      // If there's a day extension
      const newDaysNum = parseInt(changeNewDays);
      if (newDaysNum !== changeRental.days) {
        await axios.patch(`${API}/rentals/${changeRental.id}/modify-duration`, {
          new_days: newDaysNum,
          new_total: changeRental.total_amount + (changeDelta?.extensionCost || 0),
          payment_method: changePaymentMethod,
          difference_amount: changeDelta?.extensionCost || 0
        }, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
      }
      ...
      toast.success("✅ Cambio/Prórroga procesado correctamente");
      loadPendingReturns();
  ...
  const printChangeTicket = () => {
    if (!changeRental || !changeDelta) return;
    ...
    ticketWindow.document.write(`
      ...
      <style>
        ...
        <div class="header"><h1>TICKET DE REGULARIZACIÓN</h1><p>${changeAction === 'swap' ? 'Cambio de Material' : 'Prórroga'}</p></div>
        <div class="section">
        ...
        ${changeOldItem && changeNewItem ? `
        <div class="section">
          <p><strong>❌ DEVUELVE:</strong> ${changeOldItem.internal_code || changeOldItem.barcode}</p>
          <p><strong>✅ RECIBE:</strong> ${changeNewItem.internal_code || changeNewItem.barcode}</p>
        </div>` : ''}
        ${changeDelta?.extensionCost > 0 ? `
        <div class="section">
          <div class="row"><span>Días anteriores:</span><span>${changeDelta.oldDays}</span></div>
          <div class="row"><span>Días nuevos:</span><span>${changeDelta.newDays}</span></div>
          <div class="row"><span>Coste prórroga:</span><span>€${changeDelta.extensionCost.toFixed(2)}</span></div>
        </div>` : ''}
        <div class="delta-box ${changeDelta.delta > 0 ? 'delta-positive' : 'delta-negative'}">
          <p>${changeDelta.delta > 0 ? 'COBRADO' : 'ABONADO'}</p>
          <p class="delta-amount">${changeDelta.delta > 0 ? '+' : '-'}€${Math.abs(changeDelta.delta).toFixed(2)}</p>
